...
App({
   ...
   * @param cancelText //默认是“取消”
  */
  showMsgModel: function (title, content, confirmCallback, showCancel, cancelCallback, confirmText, cancelText) {
      ...
      success: function (res) {
        ...
        wx.request({
          ...
          success: res => {
            ...
            var cityData = {}
            cityData.currentCity = res.data.result.ad_info.city
            cityData.cityCode = res.data.result.ad_info.city_code.substring(3, res.data.result.ad_info.city_code.length),
              cityData.currentCounty = res.data.result.ad_info.district

            if (typeof successCallback == 'function') {
              successCallback(cityData)
            }
            // console.log(that.data.currentCityCode)
          }
        })
      }
    })
